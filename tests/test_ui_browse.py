...
def test_perms_without_account_or_rules(tmp_path, capsys):
    s = _session(tmp_path)
    assert dispatch(s, "catalog perms") is True
    out = capsys.readouterr().out
    assert "permissions by service" in out and "showing 20 of" in out
    assert dispatch(s, "catalog perms iam.serviceAccountKeys --limit 2") is True
    out = capsys.readouterr().out
    assert "iam.serviceAccountKeys.create" in out and "showing 2 of" in out
    assert dispatch(s, "catalog perms --reachable") is True  # needs an account: guard, not a crash
    assert "needs an account" in capsys.readouterr().out
    assert dispatch(s, "catalog perms --tag nope") is True
...
def test_perms_with_account_and_tag(tmp_path, capsys):
    ...
    assert dispatch(s, "catalog perms --tag PrivEsc --reachable") is True
    assert "setIamPolicy" in capsys.readouterr().out


def test_methods_permission_and_method_card(tmp_path, capsys):
    s = _session(tmp_path)
    assert dispatch(s, "catalog methods iam.serviceAccountKeys.create") is True  # works without an account
    out = capsys.readouterr().out
    assert "CreateServiceAccountKey" in out and "verified" in out
    assert dispatch(s, "catalog methods google.iam.admin.v1.CreateServiceAccountKey") is True
    out = capsys.readouterr().out
    assert "pinned fields" in out and "iam.serviceAccountKeys.create" in out
    assert dispatch(s, "catalog methods iam.serviceAccountKeys.creat") is True
    assert "did you mean" in capsys.readouterr().out


def test_roles(tmp_path, capsys):
    s = _session(tmp_path)
    assert dispatch(s, "catalog roles iam.serviceAccount --limit 3") is True
    assert "showing 3 of" in capsys.readouterr().out
    assert dispatch(s, "catalog roles --with iam.serviceAccountKeys.create --limit 3") is True
    out = capsys.readouterr().out
    assert out.index("roles/iam.serviceAccountKeyAdmin") < out.index("roles/iam.editor")  # smallest first
    assert dispatch(s, "catalog roles roles/owner iam.serviceAccountKeys") is True
    assert "iam.serviceAccountKeys.create" in capsys.readouterr().out
    assert dispatch(s, "catalog roles no/such/role") is True
...
def test_who(tmp_path, capsys):
    ...
    assert dispatch(s, "account who resourcemanager.projects.setIamPolicy") is True
    assert "admin@demo.com" in capsys.readouterr().out
    assert dispatch(s, "account who admin@demo.com") is True
    assert "setIamPolicy" in capsys.readouterr().out
    assert dispatch(s, "account who no.such.permission") is True
