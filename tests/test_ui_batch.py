...
def test_batch_check_exit_codes_and_json(tmp_path, capsys):
    ...
    assert main(base + ["check", "ok"]) == EXIT_CLEAN
    assert main(base + ["check", "bad"]) == EXIT_CLEAN  # no --fail-on: findings are reported, not fatal
    assert main(base + ["--fail-on", "finding", "check", "bad"]) == EXIT_FINDING
    assert main(base + ["--fail-on", "finding", "check", "ok"]) == EXIT_CLEAN
    capsys.readouterr()
    assert main(base + ["--json", "check", "bad"]) == EXIT_CLEAN
...
def test_batch_blindspots_report_and_script(tmp_path):
    ...
    script.write_text("# a comment\nblindspots iam.serviceAccountKeys.create\nstealth\n")
    ...
    gap = main(["--rules", str(rules), "--account", "examples/account.json", "--fail-on", "finding",
                "blindspots", "resourcemanager.projects.setIamPolicy"])
    assert gap == EXIT_FINDING


def test_batch_input_errors(tmp_path):
    assert main(["--rules", str(tmp_path / "missing.decn"), "rules"]) == EXIT_INPUT
    assert main(["--account", "tests/test_ui_batch.py", "rules"]) == EXIT_INPUT  # not JSON
...
def test_inconclusive_exit_code(tmp_path, monkeypatch):
    ...
    rules = tmp_path / "r.decn"
    rules.write_text('detection keys { event method = "google.iam.admin.v1.CreateServiceAccountKey" }\n')
    base = ["--rules", str(rules), "--account", "examples/account.json", "blindspots", "iam.serviceAccountKeys.create"]
...
def test_errors_keep_the_session_alive(tmp_path):
    ...
    assert dispatch(s, f"account {bad}") is True and s.account is None  # schema error, reported
    ...
    render.stealth = boom
    try:
        assert dispatch(s, "stealth") is True  # an unexpected exception is reported, not raised
    finally:
        render.stealth = s_orig
    assert s.account is not None and dispatch(s, "rules") is True
